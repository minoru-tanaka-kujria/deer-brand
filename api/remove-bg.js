...
export default async function handler(req, res) {
  ...
  try {
    await consumeRateLimit(db, `remove_bg_uid_${authUser.uid}`, RATE_LIMIT, RATE_WINDOW_MS);
    await consumeRateLimit(db, `remove_bg_ip_${getClientIp(req)}`, RATE_LIMIT, RATE_WINDOW_MS);
  } catch (error) {
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      return res.status(502).json({ error: "画像処理結果を取得できませんでした" });
    }
  }
  ...
  if (req.method === "POST") {
    ...
    if (!photoDataUrl) return res.status(400).json({ error: "不正なリクエストです" });
